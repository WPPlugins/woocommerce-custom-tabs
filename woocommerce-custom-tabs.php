...
Plugin URI: http://webshoplogic.com/custom-tabs-for-woocommerce/
...
$this_version = '1.0.23';
$this_plugin_name = 'Woocommerce Custom Tabs'; //can be commented out TEST

//PREV: Name: 'Woocommerce Custom Tabs' in the header and in $this_plugin_name variable
//PREV: Version: '1.0.xx PRO' in the header and in $this_version variable
//PREV: Subdir and filename: woocommerce-custom-tabs/woocommerce-custom-tabs.php (without -pro)
//PREV: More two parts signed by PREV

/*  Copyright 2015 Peter Rath WebshopLogic
...
class WCT {
	...
	function get_actual_product_related_tab_page_objects () { //$actual_post_id
		...
		usort($product_tabpage_postslist, array("WCT","product_tabpage_object_usort_compare"));		
	...
	static function product_tabpage_object_usort_compare($a, $b) {
		    return intval($a->priority) - intval($b->priority);
	}
...
// Define constasnts
	...
	define( 'WCT_VERSION', $this_version );
	define( 'WCT_NAME', $this_plugin_name );
...
// Examine if Lite and PRO versions are used at the same time. BEGIN
	...
	if ( ! function_exists ( 'wct_show_plugin_conflict_error_notice' ) ) {
		function wct_show_plugin_conflict_error_notice() {
			global $wct_conflict_plugin_names;
		
			if ( isset( $wct_conflict_plugin_names ) and is_array( $wct_conflict_plugin_names ) ) 
			foreach ($wct_conflict_plugin_names as $conflict_plugin_name ) {
				?>
				<div class="error notice">
				    <p><?php echo '<b>' . __( 'Plugin temporary disabled', 'wct' ) . ':</b> ' . $conflict_plugin_name . '<br>' . __( 'Lite and Pro or old and new versions of the plugin cannot be used at the same time! Deactivate one of them.', 'wct' ) . '<br>' . __( 'If you use an earlier and new version of PRO plugin, please use that one, that <b>name</b> contains the PRO sign. Deatcivate that one, that <b>version</b> number contains PRO.', 'wct' ); ?></p>
				</div>
				<?php			
			} 
		}	
	...
	$similar_plugin_names = array(
		'woocommerce-custom-tabs-pro/woocommerce-custom-tabs-pro.php',
		'woocommerce-custom-tabs/woocommerce-custom-tabs-pro.php',
		'woocommerce-custom-tabs/woocommerce-custom-tabs.php',
	);
	
	$possibly_conflict_plugin_names = array_diff( $similar_plugin_names, $this_plugin );
	
	//wsl_log(null, 'woocommerce-custom-tabs-pro.php $this_plugin: ' . wsl_vartotext($this_plugin));
	//wsl_log(null, 'woocommerce-custom-tabs-pro.php $similar_plugin_names: ' . wsl_vartotext($similar_plugin_names));
	//wsl_log(null, 'woocommerce-custom-tabs-pro.php $possibly_conflict_plugin_names: ' . wsl_vartotext($possibly_conflict_plugin_names));
	//wsl_log(null, 'woocommerce-custom-tabs-pro.php $active_plugins: ' . wsl_vartotext($active_plugins));
	
	global $wct_conflict_plugin_names;
	//$wct_conflict_plugin_names = array();
	...
	foreach ( $possibly_conflict_plugin_names as $possibly_conflict_plugin_name ) {
		...
		if ( $is_conflict ) {
			$wct_conflict_plugin_names[] = $possibly_conflict_plugin_name;
		}
		
	}
	
	//wsl_log(null, 'woocommerce-custom-tabs-pro.php $wct_conflict_plugin_names: ' . wsl_vartotext($wct_conflict_plugin_names));
	
	//if there is plugin conflict
	if ( isset($wct_conflict_plugin_names) and is_array($wct_conflict_plugin_names) and count($wct_conflict_plugin_names) > 0 ) {
	
		add_action( 'admin_notices', 'wct_show_plugin_conflict_error_notice' );
		
		//wsl_log(null, 'woocommerce-custom-tabs-pro.php $conflict_plugin_names 2: ' . wsl_vartotext($conflict_plugin_names));
...
//Init WCT class (only if there is no collution)
$GLOBALS['wct'] = new WCT();
