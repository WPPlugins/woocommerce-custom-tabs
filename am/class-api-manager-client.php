...
if ( get_option( AME_WCT()->ame_activated_key ) != 'Activated' ) { // MODIFY RaPe	
    //add_action( 'admin_notices', 'API_Manager_WCT::am_example_inactive_notice' );
    add_action( 'admin_notices', 'API_Manager_WCT::am_client_inactive_notice' );
}

class API_Manager_WCT { //TBSET: Change the class names so they are unique to your plugin.
	 ...
	 * @var string
	 */
	public $version = WCT_VERSION; //'1.3'; //TBSET: string - The current version of the software.

	/**
	 * @var string
	 * This version is saved after an upgrade to compare this db version to $version
	 */
	public $api_manager_client_version_name = 'plugin_api_manager_wct_version'; 	//TBSET: string - saved in the options table to determine the current version of the software to compare with a new version after a software upgrade to trigger an event.
	 ...
	 */
	public $text_domain = 'api-manager-wct_text'; 										//TBSET: string - class variable, change the value. It is better to hardcode the text domain into your plugin.
	...
	public function __construct() {
		...
		//wsl_log(null, 'WCT class-api-manager-client.php activation WCT_PLUGIN_FILE: ' . wsl_vartotext(WCT_PLUGIN_FILE));
		//wsl_log(null, 'WCT class-api-manager-client.php activation WCT_PLUGIN_BASENAME: ' . wsl_vartotext(WCT_PLUGIN_BASENAME));
		
		// Run the activation function
		//register_activation_hook( __FILE__, array( $this, 'activation' ) );
		register_activation_hook( WCT_PLUGIN_FILE, array( $this, 'activation' ) ); //TBSET // MODIFY RaPe: it is not in the plugin main file, so we need this 
		
		// Ready for translation
		////load_plugin_textdomain( $this->text_domain, false, dirname( untrailingslashit( plugin_basename( __FILE__ ) ) ) . '/languages' );
		load_plugin_textdomain( $this->text_domain, false, dirname( untrailingslashit( plugin_basename( WCT_PLUGIN_FILE ) ) ) . '/languages' ); //TBSET //MODIFY RaPe
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = WCT_NAME; //'Woocommerce Custom Tabs PRO'; 										//TBSET: string - Must match exactly, case sensitive, the Product > API tab > Software Title, as shown here: http://docs.woothemes.com/wp-content/uploads/2013/09/api-manager-simple-product-api.png

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'api_manager_wct'; 									//TBSET: string - The options table option_name that contains an option_value array which contains the ame_api_key and ame_activation_email.
			...
			$this->ame_product_id_key 			= 'api_manager_wct_product_id'; 						//TBSET: string - Is set to the Software Title value retrieved from the options table using the option_name key from $this->ame_software_product_id
			$this->ame_instance_key 			= 'api_manager_wct_instance'; 							//TBSET: string - must be unique for each activation or install. A randomly generated password will suffice.
			$this->ame_deactivate_checkbox_key 	= 'api_manager_wct_deactivate_checkbox'; 				//TBSET: option_name key used to store the option_value for the deactivation checkbox.
			$this->ame_activated_key 			= 'api_manager_wct_activated'; 							//TBSET: string - Saved in the options table to determine if the software has been activated.

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_wct_checkbox'; 					//TBSET: string - Determines the status of the deactivated checkbox.
			$this->ame_activation_tab_key 			= 'api_manager_wct_dashboard'; 						//TBSET: string - Determines the query string part of the URL for the deactivation tab.
			$this->ame_deactivation_tab_key 		= 'api_manager_wct_deactivation'; 					//TBSET: string - Determines the query string part of the URL for the activation tab.
			$this->ame_settings_menu_title 			= __('Woocommerce Custom Tabs PRO', 'api-manager-wct_text' );	//TBSET: string - Title on the settings menu.
			$this->ame_settings_title 				= __('Woocommerce Custom Tabs PRO', 'api-manager-wct_text' ); 	//TBSET: string - Title on the settings page.
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'api-manager-wct_text' ); 	//TBSET: string - Title for the activation tab.
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'api-manager-wct_text' );	//TBSET: string - Title for the deactivation tab.
			
			// ADD RaPe BEGIN
			// We do not use activation setting page, Set Activatio Key link is on the plugin list
			//$this->ame_settings_common_settings_menu_title 	= __('WebshopLogic Activation', 'api-manager-wct_text' ); 				//TBSET: WSL
			//$this->ame_settings_common_settings_title 		= __('WebshopLogic Plugin Activation Page', 'api-manager-wct_text' ); 	//TBSET: WSL
			...
			$this->ame_plugin_name 			= untrailingslashit( plugin_basename( WCT_PLUGIN_FILE ) ); //TBSET // MODIFY RaPe // same as plugin slug. if a theme use a theme name like 'twentyeleven'
		...
		register_deactivation_hook( WCT_PLUGIN_FILE, array( $this, 'uninstall' ) ); //TBSET // MODIFY RaPe
	...
	public function admin_init() { 
		...
		//wsl_log(null, 'WCT class-api-manager-client.php admin_init 0: ' . wsl_vartotext(''));
	...
	/*public function display_plugin_activation_settings_page_link() {
		...
		// http://yoursite.com/wp-admin/admin.php?page=api_manager_wct_dashboard
	...
	public function plugin_action_links( $links, $file ) {
		...
		if ( $file == WCT_PLUGIN_BASENAME ) {
			$new_links = array(
				'activate_license' => '<a href="' . admin_url() . 'admin.php?page=' . $this->ame_activation_tab_key . '">' . __('Manage License Key', AME_WCT()->text_domain ) . '</a>',
			);
	 ...
	 * API Key Class.
	 *
	 * @return  API_Manager_WCT_Key
	 */
	public function key() {
		
		//TBSET: class name, must match the class name in am/classes/class-wc-key-api.php:
		return API_Manager_WCT_Key::instance();
	}

	/**
	 * Update Check Class.
	 *
	 * @return API_Manager_WCT_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return API_Manager_WCT_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function plugin_url() {
		...
		return $this->plugin_url = plugins_url( '/', WCT_PLUGIN_FILE ); //TBSET //MODIFY RaPe
	...
	public function activation() {
		...
		//wsl_log(null, 'WCT class-api-manager-client.php activation $single_options: ' . wsl_vartotext($single_options));
	...
	public static function am_client_inactive_notice() { 
		...
		//Original: printf( __( 'The Woocommerce Custom Tabs PRO License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'api-manager-wct_text' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=api_manager_wct_dashboard' ) ) . '">', '</a>' );		
		?>
		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'api_manager_wct_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The Woocommerce Custom Tabs PRO License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'api-manager-wct_text' ), 
			'<a href="' . esc_url( admin_url() . 'admin.php?page=api_manager_wct_dashboard' ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'api-manager-wct_text' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function AME_WCT() {
    return API_Manager_WCT::instance(); 	//TBSET: API_Manager_Example - class name found inside the AME_WCT) function, must match the class name in the api-manager-example.php file.
}

// Initialize the class instance only once
AME_WCT(); 											//TBSET IN-ALL-FILES: AME_WCT) - function. Do a find and replace throughout all the plugin example files. The functions makes the singular design pattern possible in the plugin example.
